  ...
  return <section id="our-mission" className="py-24 md:py-32 relative">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="max-w-3xl mb-16">
          <span className="text-foreground/60 font-medium text-sm uppercase tracking-wider mb-4 block">WHY WE DO IT</span>
        ...
        <div className="flex justify-center items-center gap-6 mt-8">
          <button onClick={goToPrevious} className="w-12 h-12 rounded-full border border-foreground/20 flex items-center justify-center hover:bg-foreground/5 transition-colors" aria-label="Previous card">
            <ChevronLeft className="w-5 h-5 text-foreground" />
          </button>
          
          {/* Dots indicator */}
          <div className="flex gap-2">
            {missionCards.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-2 h-2 rounded-full transition-colors ${index === currentIndex ? 'bg-foreground' : 'bg-foreground/30'}`} aria-label={`Go to card ${index + 1}`} />)}
          </div>
          
          <button onClick={goToNext} className="w-12 h-12 rounded-full border border-foreground/20 flex items-center justify-center hover:bg-foreground/5 transition-colors" aria-label="Next card">
