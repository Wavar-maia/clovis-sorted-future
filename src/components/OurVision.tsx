...
const OurVision = () => {
  ...
  return <section id="our-vision" className="py-24 md:py-32 bg-[hsl(220_10%_12%)] text-white relative overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        {/* Section Header */}
        <div className="max-w-3xl mb-16">
          <span className="text-white/60 font-medium text-sm uppercase tracking-wider mb-4 block">WHY WE DO IT</span>
          <h2 className="text-3xl md:text-5xl font-bold mb-6 leading-tight text-white">Our vision</h2>
        ...
        <div className="relative">
          <div ref={scrollRef} className="flex gap-6 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-4" style={{
          scrollbarWidth: 'none',
          msOverflowStyle: 'none'
        }}>
            {visionCards.map((card, index) => <div key={index} className="flex-shrink-0 w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] snap-start">
                {/* Multicolored blur background */}
                <div className="relative rounded-3xl overflow-hidden aspect-[4/5] md:aspect-[3/4]">
                  {/* Gradient background */}
                  <div className="absolute inset-0" style={{
                background: index === 0 ? 'linear-gradient(135deg, #4a7c59 0%, #8fbc8f 30%, #f0e68c 60%, #deb887 100%)' : index === 1 ? 'linear-gradient(135deg, #2c3e50 0%, #3498db 30%, #1abc9c 60%, #f39c12 100%)' : 'linear-gradient(135deg, #667eea 0%, #764ba2 30%, #f093fb 60%, #f5576c 100%)',
                filter: 'blur(0px)'
              }} />
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            <button onClick={handlePrev} className="w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors" aria-label="Previous card">
              <ChevronLeft className="w-5 h-5 text-white" />
            </button>
            <div className="flex gap-2">
              {visionCards.map((_, index) => <button key={index} onClick={() => scrollToCard(index)} className={`w-2 h-2 rounded-full transition-colors ${currentIndex === index ? 'bg-white' : 'bg-white/30'}`} aria-label={`Go to card ${index + 1}`} />)}
            </div>
            <button onClick={handleNext} className="w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors" aria-label="Next card">
